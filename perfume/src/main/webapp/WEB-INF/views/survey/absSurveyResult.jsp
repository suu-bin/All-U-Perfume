...
<script>
	...
	<c:if test="${sessionScope.id  != null && sessionScope.kid == null}">
		<input id="btnSubmit" style="width:3cm;height:1cm; border :none; background-color:#76448A; color : white; border-radius: 10px;" type="button" value="관심향수 등록" onclick="w('${sessionScope.id}')"/>
	</c:if>
		...
		<tr>
			<c:forEach items="${result}" var="result" varStatus="status" step="1" begin="0">
				...
				<td>
				 	<br/>
				 	<br/>
					<input type="checkbox" style="width:0.5cm;height:0.5cm" name="chkBox" value = "${result.f_number}" ><br/>
					<a href="javascript:void(0);" onclick="window.open('/main/sdetail?p_number=${result.p_number}&f_name=${result.f_name}', 'pop01', 'top=10, left=10,  width=1200, height=800, status=no, menubar=no, toolbar=no, resizable=no');">
						<img src="${result.f_pic}"><br/><br/>
					</a>
					<a href="javascript:void(0);" onclick="window.open('/main/sdetail?p_number=${result.p_number}&f_name=${result.f_name}', 'pop01', 'top=10, left=10,  width=1200, height=800, status=no, menubar=no, toolbar=no, resizable=no');">
						${brand[status.index].d_name}<br/><br/>
					</a>
					<a href="javascript:void(0);" onclick="window.open('/main/sdetail?p_number=${result.p_number}&f_name=${result.f_name}', 'pop01', 'top=10, left=10,  width=1200, height=800, status=no, menubar=no, toolbar=no, resizable=no');">
						${result.f_namee}<br/><br/>
					</a>
					<br/>
					<br/>
				</td>
