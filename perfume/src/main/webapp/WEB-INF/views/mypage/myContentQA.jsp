...
<c:if test="${count == 0 }" >
	...
	<table class = "type04" style="text-align: center; ">
		<tr>
			<th colspan = "2" style = "font-size : 20px; text-align: center;"> Q & A </th>
		</tr>
		<tr>
			<th>제목</th>
			<th>등록일</th>
...
<c:if test="${count > 0 }" >
	...
	<table class = "type04" style="text-align: center; ">
			...
			<c:forEach items="${articleList}" var = "q">
				<tr>
					<td><a href="/q/list?boardnum=1">${q.subject}</a></td>
					<td>${q.reg_date}</td>
				</tr>
			</c:forEach>
			<tr>
				...
				<c:if test="${ startPage >10}" >
					<a href="/mypage/myContentQA?pageNum=${startPage-10}">[이전]</a>
				</c:if>
				<c:forEach var="g" begin="${startPage}" end="${endPage}">
					<a href="/mypage/myContentQA?pageNum=${g}">[${g}]</a>
				</c:forEach>
				<c:if test="${ endPage < pageCount}" >
					<a href="/mypage/myContentQA?pageNum=${startPage+10}">[다음]</a>
				</c:if>
				</td>
		</tr>
	</table>
