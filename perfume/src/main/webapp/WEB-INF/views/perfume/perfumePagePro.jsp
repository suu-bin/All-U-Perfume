	...
	<tr >
		...
		<td rowspan="2">
			<table   style = "width: 1200px; height : 500px; text-align : center; margin : auto; OR  ( margin-rigth : auto; margin-left : auto; )">
				...
				<tr>
					<td>
						<a href="/main/detail?p_number=${catee.p_number}&f_name=${catee.f_name}&page=${pa.page}&perPageNum=${pa.perPageNum}&gender=${catee.gender}&season=${catee.season}">
							${catee.p_number}
						</a>
					</td>
	
					<td>
						<a href="/main/detail?p_number=${catee.p_number}&f_name=${catee.f_name}&page=${pa.page}&gender=${catee.gender}&season=${catee.season}&perPageNum=${pa.perPageNum}">
							${catee.f_namee}
						</a>
					</td>
					<td style = "text-align : center;" >
						<img src="${catee.f_pic}" style="width: 70px; height : 70px;">
					</td>
				</tr>
				</c:forEach>
				<tr>
					<td colspan = "3"  >
						...
						<ul><c:if test="${c_gender != null}">
								<c:if test="${pageMaker.prev}">
									<a href="/main/search${pageMaker.makeQuery(pageMaker.startPage - 1)}&choose=name&keyword=&c_gender=${c_gender}">이전</a>
								</c:if>
							</c:if>	
							<c:if test="${c_season != null}">
								<c:if test="${pageMaker.prev}">
									<a href="/main/search${pageMaker.makeQuery(pageMaker.startPage - 1)}&choose=name&keyword=&c_season=${c_season}">이전</a>
								</c:if>
							</c:if>
							
							<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
								<c:if test="${c_gender != null}">
									<a href="/main/search?page=${idx}&perPageNum=10&choose=name&keyword=&c_gender=${c_gender}">${idx}</a>
								</c:if>
								<c:if test="${c_season != null}">
									<a href="/main/search?page=${idx}&perPageNum=10&choose=name&keyword=&c_season=${c_season}">${idx}</a>
								</c:if>
							</c:forEach>
							
							<c:if test="${c_gender != null}">
								<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
									<a href="/main/search${pageMaker.makeQuery(pageMaker.endPage+1)}&choose=name&keyword=&c_gender=${c_gender}">다음</a>
								</c:if>
							</c:if>
							<c:if test="${c_season != null}">
								<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
									<a href="/main/search${pageMaker.makeQuery(pageMaker.endPage+1)}&choose=name&keyword=&c_season=${c_season}">다음</a>
								</c:if>
							</c:if>
						</ul>
					</td>
...
<script>
<script>
  function genderlist(int page){
		location.href="http://localhost:8080/main/search?page="+page+"&perPageNum=10&choose=name&keyword=&c_gender=${c_gender}";
	}

	function seasonlist(int page){
		location.href="http://localhost:8080/main/search?page="+page+"&perPageNum=10"+"choose=name&keyword="+"&c_season=${c_season}";
	}
</script>
</body>
