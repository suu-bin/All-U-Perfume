	...
	<table class="type04" style = "text-align : center;" >
		<tr>
			<td colspan = "5"  style="text-align: right; ">
				<input  class= "inputsmall" type = "button" value = "글 쓰기" onclick="window.location='/best/write'"/>
				<c:if test="${sessionScope.id != null}">
					<input class= "inputsmall" type = "button" value = "내 작성글" onclick="window.location='/best/searchList?category=writer&input=${sessionScope.id}'"/>
				</c:if>
				<c:if test="${sessionScope.kid != null}">
					<input class= "inputsmall" type = "button" value = "내 작성글" onclick="window.location='/best/searchList?category=writer&input=${sessionScope.kid}'"/>
				</c:if>
			</td>
		</tr>
		<tr>
...
<div class="a"> 
	<table class="type04" style = "text-align : center;" >
		...
		<tr>
			...
			<td> <a href="/best/readcountUp?b_number=${boardDTO.b_number}">${boardDTO.subject}</a> </td>
		...
		<tr>
			<td colspan = "5"  style="text-align: center; ">
				<form action="/best/searchList" method="post" onsubmit="return valueCheck()">
		...
		<tr>
			<td colspan = "5"  style="text-align: center; ">
				<c:if test="${pageMaker.prev}">
					<a href="${pageMaker.makeQuery(pageMaker.startPage - 1)}&category=${category}&input=${input}">이전</a>
				</c:if>
							
				<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
					<a href="${pageMaker.makeQuery(idx)}&category=${category}&input=${input}">${idx}</a>	
				</c:forEach>
				
				<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
					<a href="list${pageMaker.makeQuery(pageMaker.endPage+1)}&category=${category}&input=${input}">다음</a>
				</c:if>
			</td>
		</tr>
	</table>
